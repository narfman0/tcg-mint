...
from . import scryfall, workspace
...
def stamp_path(cards):
    return cards.with_suffix(".stamp.json")


def local_stamp(cards):
    try:
        return json.load(open(stamp_path(cards)))
    except (FileNotFoundError, json.JSONDecodeError):
        return {}


def download(uri, cards):
...
def main(argv=None):
    ...
    cards = workspace.default().cards_file

    have = cards.exists()
    st = local_stamp(cards)
    ...
    if have and meta["updated_at"] == st.get("updated_at") and st.get("kind") == a.kind and not a.force:
        ...
        download(meta["jsonl_download_uri"], cards)
    with open(stamp_path(cards), "w") as fh:
